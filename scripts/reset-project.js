...
const carpetasAntiguas = ["app", "components", "hooks", "constants", "scripts"];
const carpetaEjemplo = "app-ejemplo";
const nuevaCarpetaApp = "app";
const rutaCarpetaEjemplo = path.join(raiz, carpetaEjemplo);
...
const moverCarpetas = async (respuesta) => {
  try {
    if (respuesta === "y") {
      await fs.promises.mkdir(rutaCarpetaEjemplo, { recursive: true });
      console.log(`📁 Carpeta /${carpetaEjemplo} creada.`);
    }

    for (const carpeta of carpetasAntiguas) {
      const rutaAntigua = path.join(raiz, carpeta);
      if (fs.existsSync(rutaAntigua)) {
        if (respuesta === "y") {
          const rutaNueva = path.join(raiz, carpetaEjemplo, carpeta);
          await fs.promises.rename(rutaAntigua, rutaNueva);
          console.log(`➡️ /${carpeta} movida a /${carpetaEjemplo}/${carpeta}.`);
        } else {
          await fs.promises.rm(rutaAntigua, { recursive: true, force: true });
    ...
    const rutaNuevaApp = path.join(raiz, nuevaCarpetaApp);
...
rl.question(
  "¿Quieres mover los archivos existentes a /app-ejemplo en lugar de eliminarlos? (Y/n): ",
  (respuesta) => {
    const entrada = respuesta.trim().toLowerCase() || "y";
    if (entrada === "y" || entrada === "n") {
      moverCarpetas(entrada).finally(() => rl.close());
    } else {
      console.log("❌ Entrada no válida. Por favor ingresa 'Y' o 'N'.");
      rl.close();
    }
  }
);
